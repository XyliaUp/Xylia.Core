...
namespace HZH_Controls.Controls
{
    ...
    public partial class UCDateTimeSelectPan : UserControl
    {
        ...
        private void SetTimeToControl()
        {
            ...
            btnYear.Text = m_dt.Year + "年";
            btnMonth.Tag = m_dt.Month;
            btnMonth.Text = m_dt.Month.ToString().PadLeft(2, '0') + "月";
            btnDay.Tag = m_dt.Day;
            btnDay.Text = m_dt.Day.ToString().PadLeft(2, '0') + "日";
            btnHour.Tag = m_dt.Hour;
            btnHour.Text = m_dt.Hour.ToString().PadLeft(2, '0') + "时";
            btnMinute.Tag = m_dt.Minute;
            btnMinute.Text = m_dt.Minute.ToString().PadLeft(2, '0') + "分";
        }
        ...
        private void SetSelectType(UCBtnExt btn)
        {
            try
            {
                ...
                if (m_thisBtn != null)
                {
                    ...
                    panTime.Source = lstSource.Select(s => s.Value).ToList();
                    panTime.SetSelect(btn.Tag);
        ...
        /// <summary>
        /// Handles the Click event of the btnTime control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
