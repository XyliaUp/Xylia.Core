...
namespace HZH_Controls.Controls
{
    ...
    public partial class UCTimePanel : UserControl
    {
        ...
        public void SetSource(List<string> lstSource)
        {
            try
            {
                ...
                if (Source != lstSource) Source = lstSource;
        ...
        public void SetSelect(object Tag)
        {
            foreach (UCBtnExt item in this.panMain.Controls)
            {
				if (item.Tag != null)
                {
                    if(item.Tag is int && (int)item.Tag == (int)Tag)
					{
                        SelectBtn = item;
                        return;
                    }
					else if(item.Tag.ToStringExt() == Tag.ToStringExt())
                    {
                        SelectBtn = item;
                        return;
                    }
