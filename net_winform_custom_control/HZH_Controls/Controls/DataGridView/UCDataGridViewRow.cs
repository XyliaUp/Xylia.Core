			...
			if (DataSource != null)
			{
				for (int i = 0; i < Columns.Count; i++)
				{
					...
					if (cs != null && cs.Length > 0)
					{
						...
						{
							cs[0].Text = com.Format is null ? row[com.DataField].ToStringExt() : com.Format(row[com.DataField]);
						}
						else
						{
							//var Value = DataSource.GetMemberVal(com.DataField);

							////如果不存在指定字段，返回原始对象
							//if (Value is null) Value = DataSource;
							//else if (com.ParamTable != null) Value = com.ParamTable.Convert(Value);

							//if (Value != null) cs[0].Text = com.Format is null ? Value.ToStringExt() : com.Format(Value);
						}
					}
				}
			}
