...
namespace HZH_Controls.Controls
{
	...
	{
		public static TreeNode getTreeNodes(this TreeNodeCollection nodeCollection, object Name)
		{
			foreach (TreeNode node in nodeCollection) if (node.Name == Name.ToString()) return node;
			return null;
		}

		public static void Remove(this TreeNodeCollection nodeCollection, object Name)
		{
			TreeNode node = getTreeNodes(nodeCollection, Name);
			if (node != null) nodeCollection.Remove(node);
		}
	}
		...
		private void treeview_DrawNode(object sender, DrawTreeNodeEventArgs e)
		{
			try
			{
				...
				else
				{
					...
					#region 绘制背景层和文本
					bool HighLight = (e.State == TreeNodeStates.Selected || e.State == TreeNodeStates.Focused || e.State == (TreeNodeStates.Focused | TreeNodeStates.Selected)) && (this._parentNodeCanSelect || e.Node.Nodes.Count <= 0);

					e.Graphics.FillRectangle(new SolidBrush(HighLight ? this.m_nodeSelectedColor : this._nodeBackgroundColor), new Rectangle(new Point(0, e.Node.Bounds.Y), new Size(base.Width, e.Node.Bounds.Height)));
					e.Graphics.DrawString(e.Node.Text, font, new SolidBrush(HighLight ? this.m_nodeSelectedForeColor : this._nodeForeColor),
						new RectangleF((float)e.Bounds.X + intLeft, (float)e.Bounds.Y + ((float)this._nodeHeight - this.treeFontSize.Height) / 2f, e.Bounds.Width, e.Bounds.Height),
						new StringFormat()
						{
					...
					if (CheckBoxes)
					{
						...
						if (e.Node.Checked && e.Node.Checked)
						{
