...
namespace HZH_Controls.Forms
{
	...
	public partial class FrmTips : FrmBase
	{
		...
		public int CloseTime
		{
			...
			{
				m_CloseTime = value;
				if (value > 0) timer1.Interval = value;
			}
		}
		...
		public static void ShowTips(string strMsg, int intAutoColseTime = 0, bool blnShowCoseBtn = true, ContentAlignment align = ContentAlignment.BottomLeft,
			Point? point = null, TipsSizeMode mode = TipsSizeMode.Small, Size? size = null, TipsState state = TipsState.Default)
		{
			var t = new Thread((ThreadStart)delegate
			{
				...
				else
				{
					...
					switch (mode)
					{
						case TipsSizeMode.Small: frmTips.Size = new Size(350, 35); break;
						case TipsSizeMode.Medium: frmTips.Size = new Size(350, 50); break;
						case TipsSizeMode.Large: frmTips.Size = new Size(350, 65); break;
						case TipsSizeMode.None:
						{
							if (!size.HasValue) frmTips.Size = new Size(350, 35);
							else frmTips.Size = size.Value;
						}
						break;
					}
					...
					if (state == TipsState.Default) frmTips.lblMsg.ForeColor = SystemColors.ControlText;
					else frmTips.lblMsg.ForeColor = Color.White;

					switch (state)
					{
						case TipsState.Default: frmTips.pctStat.Image = Properties.Resources.alarm; break;
						case TipsState.Success: frmTips.pctStat.Image = Properties.Resources.success; break;
						case TipsState.Info: frmTips.pctStat.Image = Properties.Resources.alarm; break;
						case TipsState.Warning: frmTips.pctStat.Image = Properties.Resources.warning; break;
						case TipsState.Error: frmTips.pctStat.Image = Properties.Resources.error; break;
						default: frmTips.pctStat.Image = Properties.Resources.alarm; break;
					}
		...
		public static void ReshowTips()
		{
			lock (m_lstTips)
			{
				m_lstTips.RemoveAll(p => p.IsDisposed == true);
				var enumerable = from p in m_lstTips group p by new { p.ShowAlign };
		...
		{
			if (m_lastTips.Value == this) m_lastTips = new KeyValuePair<string, FrmTips>();
		...
		/// <param name="strMsg">The string MSG.</param>
		/// <returns>FrmTips.</returns>
		public static void ShowTipsSuccess(string strMsg, int Duration = 3000)
		{
			ShowTips(strMsg, Duration, false, ContentAlignment.BottomCenter, null, TipsSizeMode.Large, null, TipsState.Success);
		}
		...
		/// <param name="strMsg">The string MSG.</param>
		/// <returns>FrmTips.</returns>
		public static void ShowTipsError(string strMsg, int Duration = 3000)
		{
			ShowTips(strMsg, Duration, false, ContentAlignment.BottomCenter, null, TipsSizeMode.Large, null, TipsState.Error);
		}
		...
		/// <param name="strMsg">The string MSG.</param>
		/// <returns>FrmTips.</returns>
		public static void ShowTipsInfo(string strMsg, int Duration = 3000)
		{
			ShowTips(strMsg, Duration, false, ContentAlignment.BottomCenter, null, TipsSizeMode.Large, null, TipsState.Info);
		}
		/// <summary>
		...
		/// <param name="strMsg">The string MSG.</param>
		/// <returns>FrmTips.</returns>
		public static void ShowTipsWarning(string strMsg, int Duration = 3000)
		{
			ShowTips(strMsg, 3000, false, ContentAlignment.BottomCenter, null, TipsSizeMode.Large, null, TipsState.Warning);
		}
